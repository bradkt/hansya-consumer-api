...
describe('WorkOrderController', function () {
    ...
    describe('RegisteredUsers', function () {
        ...
        describe('#get()', function () {
            ...
            it('should not matter what verb is used', async(function () {
                var response = await(request.delete('/workOrder/all'))
                return expect(response.statusCode).to.equal(403)
            }))
        })
    ...
    describe('admin users', function () {
        ...
        describe('#get', function () {
            it('should return all workorders', async(function () {
                var response = await(request.get('/workOrder/all'))
                return (expect(response.body.length).to.equal(3) &&
                    expect(response.statusCode).to.equal(200))
            }))

            it('should return all workorders regardless of verb & nothing should be deleted', async(function () {
                var response = await(request.delete('/workOrder/all'))
